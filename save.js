...
app  
	...
	.get("/",function(req,res){
		fs.readFile("./test.html","utf-8",function(err,file){
			res.send(file);
		});
	})
	.get("/buy",function(req,res){
		fs.readFile("./buy.html","utf-8",function(err,file){
			res.send(file);
		})
	})
	.get("/signUp",function(req,res){
		var query =req.query;
		User[query.user]=query.password;
		res.json({
			user:query.user,
	...
	.get("/signIn",function(req,res){
		var query =req.query,  
		isSuccess=+!(User[query.user]===query.password);
		res.json({
			code:isSuccess,
			data:query.user,
			message:["登录成功","登录失败"][isSuccess]
		})
	})
	.get("/getCars",function(req,res){
		res.json({
			code:0,
			data:[
